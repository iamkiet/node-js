...
exports.findOne = function (req, res) {
    ...
    celebritiesModel.findOne(req.params.celebrityId, (err, data) => {
        if (err) {
            res.status(400).send(err);
            return;
        }
...
exports.update = function (req, res) {
    ...
    celebritiesModel.update(req.params.celebrityId, req.body, (err, data) => {
        if(err){
            res.status(400).send(err);
            return
        }
...
exports.delete = function (req, res) {
    ...
    celebritiesModel.delete(req.params.celebrityId, (err, data) => {
        if(err){
            res.status(400).send(err);
            return
        }
